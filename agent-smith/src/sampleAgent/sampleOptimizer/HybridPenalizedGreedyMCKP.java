...
public class HybridPenalizedGreedyMCKP
{
	...
	public HybridPenalizedGreedyMCKP(int theCapacity, Set<Query> querySet)
	{
		System.out.println("HybridPenalizedGreedyMCKP: Capacity= " + theCapacity);
	...
	public Map<Query, Integer> calcSolution()
	{
		...
		if (capacity > 0)
		{
			...
			//System.out.println("Calculating Penalized Regular KP");
			calcPenalizedRegularKP();
			//printList(allItemsListDiffSeries);
	 ...
	 * 
	 */
	protected void calcPenalizedRegularKP()
	{ 
		double residualCapacity = capacity;
		double prevTotalValue = 0;
		double currTotalValue = 0;
		double penalizeFactor = 0.05;
		double incPenalizeFactor = 0.05;
		...
		for (int i = 0; i < allItemsListDiffSeries.size(); i++)
		{
			if((allItemsListDiffSeries.get(i).getWeight()) <= residualCapacity)
			{
				...
				prevTotalValue = prevTotalValue + allItemsListDiffSeries.get(i).getValue();
		...
		for (int i = lastIndexReached+1; i < allItemsListDiffSeries.size(); i++)
		{
			currTotalValue = 0;
			for (int j = 0; j <= i; j++)
			{
				currTotalValue = currTotalValue + (allItemsListDiffSeries.get(j).getValue() * (1.0-penalizeFactor));
			}
			
			if(currTotalValue<prevTotalValue)
			{
				break;
			}
			else
			{
				...
				penalizeFactor += incPenalizeFactor;
